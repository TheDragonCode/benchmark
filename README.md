...
<picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://banners.beyondco.de/Benchmark.png?pattern=topography&style=style_2&fontSize=100px&md=1&showWatermark=1&theme=dark&packageManager=composer+require+--dev&packageName=dragon-code%2Fbenchmark">
    <img src="https://banners.beyondco.de/Benchmark.png?pattern=topography&style=style_2&fontSize=100px&md=1&showWatermark=1&theme=light&packageManager=composer+require+--dev&packageName=dragon-code%2Fbenchmark">
</picture>
...
If the regression exceeds the threshold, an `AssertionError` is thrown with a message like:

```
The time regression value must be less than or equal to 15%. Current value: 23.45%.
```
...
```
The memory regression value must be less than or equal to 15%. Current value: 42.00%.
```

#### Using Both Assertions Together

Both regression assertions can be chained along with any other `toAssert` assertions:
...
new Benchmark()
    ...
    ->toAssert()
    ->toBeRegressionTime(max: 10)   // no more than 10% time regression
    ->toBeRegressionMemory(max: 10); // no more than 10% memory regression
```
